...
function plot_tracking_results((x_akmpc,u_akmpc,t_akmpc),(x_skmpc,u_skmpc,t_skmpc),  (x_lmpc,u_lmpc,t_lmpc), (x_ref, t_ref))
    ...
    elseif size(x_akmpc)[1] == 4
        fig = Figure(fontsize = 30, size=(2500, 1100))
        ga = fig[1:2,1:2]= GridLayout() 
        ax1a = Axis(ga[1,1], xlabel=L"$t$ (s)",ylabel=L"$θ_2$ (rad)", width=1000,height=500,ytickformat = "{:.1f}")
        # θ_2
        ...
        # u_2 
        ax2a= Axis(ga[2,1], xlabel=L"$t$ (s)",ylabel=L"$u_2$ (Nm)", width=1000,height=500,ytickformat = "{:.1f}")
        ...
        axislegend(ax1a, [x1r, x1l,x1a, x1s], [L"$$reference", L"$$linearization MPC",L"$$adaptive KMPC", L"$$static KMPC"])
        hidexdecorations!(ax1a, ticks = true, grid=false)

        # θ_1
        ax1b = Axis(ga[1,2], xlabel=L"t (s)",ylabel=L"$θ_1$ (rad)", width=1000,height=500,ytickformat = "{:.1f}")
        ...
        # u_1 
        ax2b= Axis(ga[2,2], xlabel=L"t (s)",ylabel=L"$u_1$ (Nm)", width=1000,height=500,ytickformat = "{:.1f}")
        x1a = lines!(ax2b, t_akmpc[1:end-1], u_akmpc[1,:], color=red_plot,linewidth=linew)
        x1s = lines!(ax2b, t_skmpc[1:end-1], u_skmpc[1,:], color="gray",linewidth=linew)

...
function plot_tracking_results((x_akmpc,u_akmpc,t_akmpc),(x_skmpc,u_skmpc,t_skmpc),  (x_lmpc,u_lmpc,t_lmpc), (x_ref, t_ref))
    ...
    if size(x_akmpc)[1] == 2
        ...
        # Legend(fig[1,1], [x1r, x1l, x1a, x1s], [L"$$reference", L"$$linearization MPC", L"$$adaptive 
        # KMPC", L"$$static KMPC"], orientation = :horizontal)
    ...
    elseif size(x_akmpc)[1] == 4
        ...
        ax1a = Axis(ga[3,1], xlabel=L"$t$ (s)",ylabel=L"$θ_2$ (rad)", width=1000,height=225,ytickformat = "{:.1f}")
        # θ_2
